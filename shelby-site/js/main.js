(function(){
  ...
  function icon(name){
    const icons = {whatsapp:'☎', prev:'‹', next:'›', close:'×'};
    return icons[name] || '';
  }
  ...
  function productCard(product, mini=false){
    ...
    const label = createEl('span','card-focus-label',`Seguro ${product.name}`);
    card.append(info, quote, label);
    return card;
  }
  ...
  function initProductsSection(){
    ...
    function select(cat){
      selected = cat;
      $$('.tab', tabs).forEach((t,i)=>t.classList.toggle('is-active', DATA.categories[i].slug === cat.slug));
  ...
  function initContactForm(){
    ...
    if(productSelect){
      DATA.products.forEach(p => productSelect.appendChild(new Option(p.name, p.name)));
    }
    form.addEventListener('submit', e => {
      e.preventDefault();
  ...
  function initProductPage(){
    ...
    quoteButtons.forEach(btn => btn.addEventListener('click', () => handleQuote(product)));
    ...
    if(product.coverages.length){
      product.coverages.forEach(c => {
        ...
        card.innerHTML = `<img src="${c.icon}" alt="${c.title}" loading="lazy">${hover}<span class="coverage-title-fallback">${c.title}</span>`;
        coverageTrack.appendChild(card);
      });
    } else {
      $('.coverage-section').classList.add('hidden');
    }
    $('#coverage-prev')?.addEventListener('click',()=>coverageTrack.scrollBy({left:-coverageTrack.clientWidth,behavior:'smooth'}));
    $('#coverage-next')?.addEventListener('click',()=>coverageTrack.scrollBy({left:coverageTrack.clientWidth,behavior:'smooth'}));
